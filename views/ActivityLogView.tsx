                ...
                <form onSubmit={handleSubmit} className="p-4 space-y-4 overflow-y-auto">
                    <div>
                        <div className="flex gap-4 p-1 bg-gray-200 dark:bg-gray-600 rounded-full">
                            <button type="button" onClick={() => setType(TransactionType.Expense)} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === TransactionType.Expense ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('expense')}</button>
                            <button type="button" onClick={() => setType(TransactionType.Income)} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === TransactionType.Income ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('income')}</button>
                        </div>
                    </div>
                    <div>
...
const ActivityItem: React.FC<{ item: AllTransaction }> = ({ item }) => {
    ...
    return (
        <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 p-4 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] flex items-center gap-4">
