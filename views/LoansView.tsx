                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <div className="flex gap-4 p-1 bg-gray-200 dark:bg-gray-600 rounded-full">
                            <button type="button" onClick={() => setType(LoanType.Lent)} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === LoanType.Lent ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('lent')}</button>
                            <button type="button" onClick={() => setType(LoanType.Borrowed)} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === LoanType.Borrowed ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('borrowed')}</button>
                        </div>
                    </div>
                    <div>
...
const LoansView: React.FC<LoansViewProps> = ({ loans, addLoan, deleteLoan, addRepayment }) => {
    ...
    return (
        <div className="p-4 sm:p-6 bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 rounded-3xl shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] animate-fadeIn">
