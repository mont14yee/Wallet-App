            ...
            <form onSubmit={handleSubmit} className="space-y-4">
                 <div>
                    <div className="flex gap-4 p-1 bg-gray-200 dark:bg-gray-600 rounded-full">
                        <button type="button" onClick={() => { setType(TransactionType.Expense); setCategory(''); }} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === TransactionType.Expense ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('expense')}</button>
                        <button type="button" onClick={() => { setType(TransactionType.Income); setCategory(''); }} className={`flex-1 py-1 rounded-full text-sm font-semibold ${type === TransactionType.Income ? 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50 text-slate-700 dark:text-slate-200 shadow' : ''}`}>{t('income')}</button>
                    </div>
                </div>
...
const ScheduledItemCard: React.FC<{
    ...
    return (
        <div className={`p-4 rounded-lg shadow-[0_8px_30px_rgb(0,0,0,0.04)] dark:shadow-[0_8px_30px_rgb(0,0,0,0.2)] flex flex-col sm:flex-row items-start sm:items-center gap-4 transition-all duration-300 ${isDue ? 'bg-yellow-50 dark:bg-yellow-900/40 border-l-4 border-yellow-400' : 'bg-white/80 dark:bg-gray-800/80 backdrop-blur-xl border border-white/60 dark:border-gray-700/50'}`}>
