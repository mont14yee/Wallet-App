...
const ResultDisplay: React.FC<{ title: string; children: React.ReactNode; icon: string; }> = ({ title, children, icon }) => (
    <div className="mt-6 p-5 bg-gradient-to-br from-slate-50 to-gray-100 dark:from-gray-700 dark:to-gray-800 rounded-3xl shadow-inner">
        ...
        <div className="p-3 sm:p-4 flex flex-col h-full bg-gray-100 dark:bg-gray-800/50">
            ...
            <div className="grid grid-cols-4 gap-3 flex-grow">
                {buttons.map(btn => {
                    ...
                    return <button key={btn} onClick={() => handleButtonClick(btn)} className={`p-2 text-2xl font-semibold rounded-3xl transition-all duration-150 active:transform active:scale-95 h-full ${style}`}>
...
const Calculator: React.FC = () => {
    ...
    if (!activeCalculator) {
        return (
            <div className="p-4 sm:p-6 h-full bg-transparent rounded-3xl">
