...
code = code.replace(/import React, \{ useState, useEffect \} from 'react';import \{ useLanguage \} from '\.\.\/\.\.\/contexts\/LanguageContext';import \{ UserProfile \} from '\.\.\/\.\.\/types';/, importReplacement);
...
code = code.replace(/    return \(\n        <div className="pb-24 animate-fade-in px-4">\n            <h1 className="text-2xl font-bold text-gray-800 dark:text-gray-100 mb-6">\{t\('settings'\)\}<\/h1>\n            \n            <SettingsSection title=\{t\('profile'\)\} icon="fas fa-user-circle">\n[\s\S]*?<\/SettingsSection>/, newProfileSection);
